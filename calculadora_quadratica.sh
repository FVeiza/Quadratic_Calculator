...
if [ "$coefa" -ne 0 ]; then
 ...
 echo -n "X1 = "
 echo "scale=3; (-1*$coefb)/(2*$coefa)" | bc -l | awk '{printf
"%.3f\n", $0}'
 echo -n "X2 = "
 echo "scale=3; (-1*$coefb)/(2*$coefa)" | bc -l | awk '{printf
"%.3f\n", $0}'
 elif [ $(echo "$coefb*$coefb-4*$coefa*$coefc" | bc -l) -lt 0 ]; then
 echo "Como o valor de delta é menor que 0, a equação não apresenta
nenhuma raiz real."
 fi
 echo ""
 echo "====================================="
 ...
 case "$control" in
 1)
 termod1=$(echo "2*$coefa" | bc -l | awk '{printf
"%.3f\n", $0}')
 ...
 echo ""
 echo "Por favor, selecione uma opção para continuar:"
 echo "1) Exibir derivada de primeira ordem da
equação"
 echo "2) Exibir integral indefinida da equação"
 echo "3) Resolver outra equação"
 echo "4) Sair"
 ;;
 2)
 termoi1=$(echo "scale=3; $coefa/3" | bc -l | awk
'{printf "%.3f\n", $0}')
 termoi2=$(echo "scale=3; $coefb/2" | bc -l | awk
'{printf "%.3f\n", $0}')
 echo "Calculando integral indefinida..."
 echo "∫f(x)dx = ($termoi1)x^3 + ($termoi2)x^2 +
($coefc)x + C"
 echo ""
 echo "====================================="
 echo ""
